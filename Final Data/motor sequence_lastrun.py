...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\91982\\Downloads\\Psychopy\\experiment 6\\motor sequence_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixation_2 = visual.ShapeStim(
        ...
        opacity=None, depth=0.0, interpolate=True)
    line_01 = visual.Rect(
        win=win, name='line_01',units='pix', 
        width=(100,10)[0], height=(100,10)[1],
        ori=90.0, pos=(-150,0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-1.0, interpolate=True)
    line_02 = visual.Rect(
        win=win, name='line_02',units='pix', 
        width=(100,10)[0], height=(100,10)[1],
        ori=90.0, pos=(-50,0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-2.0, interpolate=True)
    line_03 = visual.Rect(
        win=win, name='line_03',units='pix', 
        width=(100,10)[0], height=(100,10)[1],
        ori=90.0, pos=(50,0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-3.0, interpolate=True)
    line_04 = visual.Rect(
        win=win, name='line_04',units='pix', 
        width=(100,10)[0], height=(100,10)[1],
        ori=90.0, pos=(150,0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('random_2.started', globalClock.getTime(format='float'))
        ...
        _key_resp2_allKeys = []
        # keep track of which components have finished
        random_2Components = [fixation_2, line_01, line_02, line_03, line_04, probe_2, key_resp2]
        ...
        while continueRoutine:
            ...
            # if line_01 is starting this frame...
            if line_01.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line_01.frameNStart = frameN  # exact frame index
                line_01.tStart = t  # local t and not account for scr refresh
                line_01.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line_01, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line_01.started')
                # update status
                line_01.status = STARTED
                line_01.setAutoDraw(True)
            
            # if line_01 is active this frame...
            if line_01.status == STARTED:
                # update params
                pass
            
            # *line_02* updates
            
            # if line_02 is starting this frame...
            if line_02.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line_02.frameNStart = frameN  # exact frame index
                line_02.tStart = t  # local t and not account for scr refresh
                line_02.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line_02, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line_02.started')
                # update status
                line_02.status = STARTED
                line_02.setAutoDraw(True)
            
            # if line_02 is active this frame...
            if line_02.status == STARTED:
                # update params
                pass
            
            # *line_03* updates
            
            # if line_03 is starting this frame...
            if line_03.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line_03.frameNStart = frameN  # exact frame index
                line_03.tStart = t  # local t and not account for scr refresh
                line_03.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line_03, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line_03.started')
                # update status
                line_03.status = STARTED
                line_03.setAutoDraw(True)
            
            # if line_03 is active this frame...
            if line_03.status == STARTED:
                # update params
                pass
            
            # *line_04* updates
            
            # if line_04 is starting this frame...
            if line_04.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line_04.frameNStart = frameN  # exact frame index
                line_04.tStart = t  # local t and not account for scr refresh
                line_04.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line_04, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line_04.started')
                # update status
                line_04.status = STARTED
                line_04.setAutoDraw(True)
            
            # if line_04 is active this frame...
            if line_04.status == STARTED:
                # update params
                pass
            ...
            if key_resp2.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp2.getKeys(keyList=None, ignoreKeys=["escape"], waitRelease=False)
        ...
        for thisComponent in random_2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('random_2.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        trials_2.addData('key_resp2.corr', key_resp2.corr)
        if key_resp2.keys != None:  # we had a response
            trials_2.addData('key_resp2.rt', key_resp2.rt)
            trials_2.addData('key_resp2.duration', key_resp2.duration)
